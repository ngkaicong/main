...
public class VersionedEntriesBookTest {

    private final ReadOnlyEntriesBook addressBookWithAmy = new EntriesBookBuilder().withEntry(AMY).build();
    private final ReadOnlyEntriesBook addressBookWithBob = new EntriesBookBuilder().withEntry(BOB).build();
    private final ReadOnlyEntriesBook addressBookWithCarl = new EntriesBookBuilder().withEntry(IDA).build();
    private final ReadOnlyEntriesBook emptyAddressBook = new EntriesBookBuilder().build();

    @Test
    public void commit_singleAddressBook_noStatesRemovedCurrentStateSaved() {
        VersionedEntriesBook versionedAddressBook = prepareAddressBookList(emptyAddressBook);
    ...
    public void commit_multipleAddressBookPointerAtEndOfStateList_noStatesRemovedCurrentStateSaved() {
        VersionedEntriesBook versionedAddressBook = prepareAddressBookList(
    ...
    public void undo_singleAddressBook_throwsNoUndoableStateException() {
        ...
        assertThrows(VersionedEntriesBook.NoUndoableStateException.class, versionedAddressBook::undo);
    }
    ...
    public void redo_singleAddressBook_throwsNoRedoableStateException() {
        ...
        assertThrows(VersionedEntriesBook.NoRedoableStateException.class, versionedAddressBook::redo);
    }
    ...
    @Test
    public void equals() {
        VersionedEntriesBook versionedAddressBook = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);

        // same values -> returns true
        VersionedEntriesBook copy = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);
        ...
        VersionedEntriesBook differentAddressBookList = prepareAddressBookList(addressBookWithBob, addressBookWithCarl);
        assertFalse(versionedAddressBook.equals(differentAddressBookList));

        // different current pointer index -> returns false
        VersionedEntriesBook differentCurrentStatePointer = prepareAddressBookList(
     ...
     */
    private void assertAddressBookListStatus(VersionedEntriesBook versionedAddressBook,
                                             List<ReadOnlyEntriesBook> expectedStatesBeforePointer,
                                             ReadOnlyEntriesBook expectedCurrentState,
                                             List<ReadOnlyEntriesBook> expectedStatesAfterPointer) {
        // check state currently pointing at is correct
        assertEquals(new EntriesBook(versionedAddressBook), expectedCurrentState);
        ...
        for (ReadOnlyEntriesBook expectedAddressBook : expectedStatesBeforePointer) {
            assertEquals(expectedAddressBook, new EntriesBook(versionedAddressBook));
            versionedAddressBook.redo();
        }

        // check states after pointer are correct
        for (ReadOnlyEntriesBook expectedAddressBook : expectedStatesAfterPointer) {
            versionedAddressBook.redo();
            assertEquals(expectedAddressBook, new EntriesBook(versionedAddressBook));
        }
    ...
    /**
     * Creates and returns a {@code VersionedEntriesBook} with the {@code addressBookStates} added into it, and the
     * {@code VersionedEntriesBook#currentStatePointer} at the end of list.
     */
    private VersionedEntriesBook prepareAddressBookList(ReadOnlyEntriesBook... addressBookStates) {
        assertFalse(addressBookStates.length == 0);

        VersionedEntriesBook versionedAddressBook = new VersionedEntriesBook(addressBookStates[0]);
     ...
     */
    private void shiftCurrentStatePointerLeftwards(VersionedEntriesBook versionedAddressBook, int count) {
