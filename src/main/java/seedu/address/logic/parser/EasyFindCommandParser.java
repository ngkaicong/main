 ...
 */
public class EasyFindCommandParser implements Parser<Filter> {
    /**
    ...
    public Filter parse(String args) throws ParseException {
        ...
        if (prefix.equals("n/")) {
            return new Filter(new NameContainsKeywordsPredicate(Arrays.asList(keyWords)));
        } else if (prefix.equals("d/")) {
            return new Filter(new DateContainsSpecifiedKeywordsPredicate(Arrays.asList(keyWords)));
        } else if (prefix.equals("t/")) {
            return new Filter(new TagContainsSpecifiedKeywordsPredicate(Arrays.asList(keyWords)));
        } else {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, Filter.MESSAGE_USAGE));
        }
    }
