...
We have implemented dropdown menu UI for autocomplete with a list of commands prompts.
                    **Alternative 1 (Current Choice):** Include dropdown menu to list all autocomplete options +
                    ...
                    **Alternative 2 :** No Dropdown Menu
...
* if search by `cashflow` then `CashFlowContainsSpecifiedKeywordsPredicate` is used
...
* There will be no results returned in this scenario as input date does not match the phone numbers in `Entry 1`
...
**Aspect:** Easy Search with other details. +
...
It represents an in-memory model of the AddressBook and is the component which manages the interactions between
the commands and the `VersionedAddressBook`.
...
==== Current Implementation
The Budgeter allows a report command that can generate a report.

Type detailed report and u will find a detailed report.

==== Design Considerations

===== Aspect: Password length and strength

* **Current Design:** Report will show a piechart
...
|`* *` |user |generate a financial report |To view a consolidated report for the time period about income and expenditure

|`*` |user |export financial report as an Microsoft Excel Spreadsheet |To generate an excel file of the report generated for further data visualisation / augmentation on the Microsoft Excel platform
|=======================================================================
...
2.  Financial Saviour displays income details and prompt the user for confirmation
3.  User checks the displayed income details and confirms that the details are correct
4.  Financial Saviour creates a new income entry within itself
+
Use case ends.
