...
Format: `edit INDEX [n/NAME] [d/DATE] [e/CASHFLOW] [t/TAG]...`
...
=== Showing a summary table of all financial activity by category: `summary`

Shows a summary table listing by categories. +
...
the *net cash flow calculated.* A `category` refers to any `set of tags` that are assigned to any entries in the application. +

The summary command supports both listing categories. +
Format: +
...
* For the command *"summary category"*, START_DATE/END_DATE must be in the format of
...
The table entries are sorted according to category depending on the mode used in ascending order. +
At the same time, currently selected entry will be unselected to reduce confusion for the user.

[NOTE]
The table listing is currently non-resizable. If category is too long, the default behaviour of this app is to truncate
...
* *Display* : `display`
* *Lock* : `lock`
* *Summary by category* : `summary category [d/Start date End date]`
=======
