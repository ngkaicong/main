...
The current version of setting the password is not instant so after locking the Budgeteer,
one is advised to close the application and restart the application.
After opening the app, the data entries will be locked and prevented from being edited.
To remove or change the passwords, please refer to the examples below:
